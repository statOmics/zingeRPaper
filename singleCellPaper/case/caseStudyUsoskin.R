...
pdf("~/Dropbox/phdKoen/singleCell/usoskinweights1To10BatchInMainAndZeroModelAllCellTypes_v2.pdf")
...
dNoWeights=calcNormFactors(dNoWeights)
...
resList=list()
for(i in 1:ncol(L2)) resList[[i]] = results(dse,contrast=L2[,i])
#save(resList,file="/Users/koenvandenberge/Dropbox/PhD/Research/zeroInflation/singleCell/resListUsoskin.rda")
load("/Users/koenvandenberge/Dropbox/PhD/Research/zeroInflation/singleCell/resListUsoskin.rda")
...
setwd("~/Dropbox/PhD/Research/zeroInflation/singleCell/usoskin/DESeq2Zero/")
...
## analysis
load("~/Dropbox/PhD/Research/zeroInflation/singleCell/usoskin/DESeq2Zero/weightsDESeq2ZeroUsoskin31.RData") #300 iterations
hist(w[usoskin==0])
library(DESeq2) ; library(genefilter)
dse = DESeqDataSetFromMatrix(usoskin, colData=data.frame(cellType=cellTypeAll, pickingSession=pickingSession), design=~cellType+pickingSession)
dse = DESeq2::estimateSizeFactors(dse, type = "poscounts")
dimnames(w) = NULL
assays(dse)[["weights"]] = w
dse <- DESeq(dse, betaPrior=TRUE)
#save(dse,file="/Users/koenvandenberge/Dropbox/PhD/Research/zeroInflation/singleCell/dseDESeq2Zero300IterUsoskin.rda")
resultsNames(dse) #for building contrasts, see ?results
L2=matrix(0,nrow=length(resultsNames(dse)),ncol=11)
rownames(L2)=resultsNames(dse)
L2[2:12,1] = c(1,rep(-1/10,10))
...
for(i in 1:ncol(L2)){
	cat(i)
	resList[[i]] = results(dse,contrast=L2[,i])
	} 
#save(resList,file="/Users/koenvandenberge/Dropbox/PhD/Research/zeroInflation/singleCell/resListDESeq2ZeroUsoskin.rda")
load("/Users/koenvandenberge/Dropbox/PhD/Research/zeroInflation/singleCell/resListDESeq2ZeroUsoskin.rda")
